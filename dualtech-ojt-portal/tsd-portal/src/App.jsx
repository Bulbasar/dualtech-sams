...
import React, { useState, useEffect, useMemo, useRef, useCallback } from "react";
        ...
        import { 
            ...
            Phone, Plus, Trash2, Crosshair, Navigation, Building, CalendarCheck, Loader2, TrendingUp, Menu, FileUp, ArrowLeft, Presentation, Video, BarChart2, Edit3, ClipboardList, Sun, Moon, Layers, DollarSign, Lock, EyeOff, Eye, CheckCircle, ChevronUp, Columns, ListChecks, Loader
        ...
        <div className="min-h-screen relative flex items-center justify-center p-4 bg-slate-900 overflow-hidden font-sans">
            ...
            <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-700">
                ...
                <div className="mt-8 text-center animate-in fade-in delay-300 duration-1000">
                    <p className="text-xs text-slate-500 font-medium">Technical Services Division &copy; {new Date().getFullYear()} Dualtech Training Center</p>
                </div>
            </div>
        </div>
    );
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {(() => {
                                        if (!selectedCompany) {
                                            return <tr><td colSpan="7" className="p-8 text-center text-slate-400">Please select a company to view attendance records.</td></tr>;
                                        }
                                        if (loadingLogs && filteredRecords.length === 0) {
                                            return <tr><td colSpan="7" className="p-8 text-center text-emerald-600 font-bold">Fetching logs for {selectedCompany}...</td></tr>;
                                        }
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                {sortConfig.key === sortKey && (
                                                                    <span className="ml-1 text-primary-500">{sortConfig.direction === 'asc' ? 'â†‘' : 'â†“'}</span>
                                                                )}
                                                            </th>
                                                        );
                                                    })}
...
const CompanyAttendanceView = ({ currentUser, globalSearchPreFill, setGlobalSearchPreFill }) => {
                ...
                <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-900 transition-colors">
                    ...
                    <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white dark:bg-slate-900">
                        ...
                        <header className="h-16 flex items-center justify-between px-4 md:px-6 border-b border-slate-200 dark:border-slate-700 relative z-30">
                            ...
                            <div className="flex items-center gap-4"><NotificationBell db={db} APP_ID={APP_ID} currentUser={user} setActiveView={setActiveView} /><div className="text-slate-500 text-xs md:text-sm hidden sm:block">Signed in as <span className="font-semibold text-slate-800 dark:text-slate-200">{user?.email}</span></div>
...
{!selectedProject ? (
    <Routes>
        ...
        <Route path="/company_attendance" element={<CompanyAttendanceView currentUser={user} globalSearchPreFill={globalSearchQuery} setGlobalSearchPreFill={setGlobalSearchQuery} />} />
        <Route path="/ojtAttendance" element={<OjtAttendanceView />} /><Route path="/visitSchedule" element={<VisitScheduleView db={db} APP_ID={APP_ID} currentUser={user} />} /><Route path="/engagement" element={<CompanyEngagementView />} />
