...
// Extract OnlineSchoolingSubmissionsTab from mentoring
const mentoringRegex = /function OnlineSchoolingSubmissionsTab\(\)\s*\{([\s\S]*?)return\s*\(\s*([\s\S]*?)\s*\);\s*\}/;
const mentoringMatch = mentoringContent.match(mentoringRegex);

if (!mentoringMatch) {
    console.error("Could not find OnlineSchoolingSubmissionsTab in mentoring.html");
    process.exit(1);
}

const uiBody = mentoringMatch[2]; // the return (...) content
...
const replaced = schoolingContent.replace(schoolingRegex, newOnlineSubmissionsTab);

if(replaced === schoolingContent) {
    console.error("Failed to replace: regex did not match schooling.html!");
} else {
    fs.writeFileSync(schoolingFile, replaced);
    console.log("Successfully copied OnlineSchoolingSubmissionsTab format to schooling.html. New length: " + replaced.length);
}
