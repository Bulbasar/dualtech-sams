...
import { collection, getDocs, getDoc, doc, query, where, limit, deleteDoc, addDoc, orderBy } from "firebase/firestore";
...
export default function BSTPHomeTab({ user, profile, onRefresh }) {
    ...
    const handleActionClick = async (type) => {
        ...
        if (type === 'IN' || type === 'OUT') {
            ...
            // --- BIOMETRIC / PIN AUTHENTICATION (For Time IN) ---
            if (type === 'IN') {
            ...
            setConfirmModalState({ type, targetGeoResult, ...config[type], isOffSite });
            setRemarks("");
            setDeviceExpl("");
        }
    };
        ...
        <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-16">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-800 flex justify-between items-center relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">{profile?.name || "Trainee"}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-2">
                ...
                {hasTimedIn && !hasTimedOut && !activeSkillsetIn && (
                    <button 
