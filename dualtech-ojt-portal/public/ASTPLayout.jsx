...
export default function ASTPLayout({ user, profile, activeTab, setActiveTab, allLogs = [], absenceLogs = [], onRefresh, children }) {
    ...
    useEffect(() => {
        ...
        const fetchAnnouncements = async () => {
            try {
                ...
                // Calculate Missing Working Days and Total Absences
                let start = null;
        ...
        <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col relative font-sans">
            ...
            <header className="sticky top-0 z-50 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm h-14 flex items-center justify-between px-4">
                ...
                <div className="flex items-center justify-end gap-2 w-1/4">
                    ...
                    <button onClick={() => setShowScheduleModal(true)} className="relative w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors" title="Update Schedule">
                    ...
                    <button onClick={() => setShowMessageModal(true)} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 hover:bg-slate-200 transition-colors">
            ...
            <div className="flex-1 flex max-w-[1600px] mx-auto w-full pt-4">
                ...
                <div className="hidden md:flex w-[280px] flex-col sticky top-[72px] h-[calc(100vh-80px)] overflow-y-auto px-2 pb-4">
                    ...
                    <nav className="space-y-1">
                        ...
                        <button onClick={() => setActiveTab('absence')} className={`w-full flex items-center gap-3 p-3 rounded-lg font-medium transition-colors ${activeTab === 'absence' ? 'bg-slate-200 dark:bg-slate-800' : 'text-slate-700 hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-800'}`}><FileText size={24} className={activeTab === 'absence' ? 'text-blue-500' : 'text-slate-500'} /> Absences</button>
                        <button onClick={() => setActiveTab('schooling')} className={`w-full flex items-center gap-3 p-3 rounded-lg font-medium transition-colors ${activeTab === 'schooling' ? 'bg-slate-200 dark:bg-slate-800' : 'text-slate-700 hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-800'}`}><BookOpen size={24} className={activeTab === 'schooling' ? 'text-blue-500' : 'text-slate-500'} /> Schooling</button>
                        ...
                        <button onClick={() => setActiveTab('profile')} className={`w-full flex items-center gap-3 p-3 rounded-lg font-medium transition-colors ${activeTab === 'profile' ? 'bg-slate-200 dark:bg-slate-800' : 'text-slate-700 hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-800'}`}><Settings size={24} className={activeTab === 'profile' ? 'text-slate-500 fill-slate-500/20' : 'text-slate-500'} /> Profile Settings</button>
                    </nav>
                ...
                <div className="hidden lg:block w-[320px] sticky top-[72px] h-[calc(100vh-80px)] overflow-y-auto px-2 pb-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-800 p-4 mb-4">
            ...
            <div className="md:hidden fixed bottom-0 left-0 w-full z-50 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-around py-1 px-1 sm:px-2 shadow-lg">
                {[
